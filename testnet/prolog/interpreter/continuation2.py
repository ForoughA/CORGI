...
class Engine(object):
    def __init__(self, load_system=False, similarity=None, max_depth=20, lambda_cut=0.1, embeddings = None, rule2index = None, maxk = 3):
    ...
    def call(self, query, rule, scont, fcont, heap, k = 0):
        ...
        scont, fcont, heap = _make_rule_conts(self, scont, fcont, heap, query, rule, k, rule_score)
        return scont, fcont, heap
...
def _log_meta_info(heap, rule):
    ...
    """heap.rules.append("*RULE USED")
    heap.rules.append('|'+str(heap.score())+'|'+rule.source)
    heap.rules.append("UNIFIED")
    heap.rules += heap.unifications
    heap.rules.append("AT DEPTH")
    heap.rules.append(str(heap.depth))
    heap.unifications = []"""

def _make_rule_conts(engine, scont, fcont, heap, query, rule, k, score):
    ...
    if k < self.maxk:
        ...
        query_string = self.get_query_string(query)
        query_embedding = self.get_query_embedding(querystring)
        rule_index, nextrule_score = self.get_similar_rule(query_embedding, k) 
        nextrule = self.index2rule[rule_index]

        k = k+1

        fcont = UserCallContinuation.make(query.arguments(), engine, scont, fcont, query, nextrule, k, nextrule_score)
        heap = heap.branch()

    try:
        shared_env = rule.unify_and_standardize_apart_head(heap, query, similarity=engine.similarity, score)
        #print "shared env", shared_env
    except error.UnificationFailed:
...
class UserCallContinuation(FailureContinuation):
    def __init__(self, engine, nextcont, orig_fcont, heap, query, rule, k, score):
